...
async def aii(client: Client, message):
   ...
   try:
    ...
    chat_id = message.chat.id
    user_id = message.from_user.id if message.from_user else "CRAZ_UP"
    videoid = None
    link = None
...
async def play(client: Client, message):
  ...
  gr = await get_group(bot_username)
  ch = await get_channel(bot_username)
  button = [[InlineKeyboardButton(text=".𝗘𝗡𝗗", callback_data=f"stop"), InlineKeyboardButton(text="𝗥𝗘𝗦𝗨𝗠𝗘", callback_data=f"resume"), InlineKeyboardButton(text="𝗣𝗔𝗨𝗦𝗘", callback_data=f"pause")], [InlineKeyboardButton(text="𝗖𝗛𝗔𝗡𝗘𝗘𝗟🖱", url=f"{ch}"), InlineKeyboardButton(text="𝗚𝗥𝗢𝗨𝗣🖱", url=f"{gr}")], [InlineKeyboardButton(text=f"{OWNER_NAME}", url="https://t.me/CRAZ_UP")], [InlineKeyboardButton(text="اضف البوت الي مجموعتك او قناتك 🚦🚀", url=f"https://t.me/{bot_username}?startgroup=True")]]
     ...
     if await is_served_call(client, message.chat.id):
         ...
         if message.from_user:
          if message.from_user.photo:
           ...
           ahmed = await client.get_chat("CRAZ_UP")
