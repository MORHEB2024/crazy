...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "CRAZ_UP":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "كريزي")
      except:
        pass
      return await message.reply_text(f"**♪ انضم المطو كريزي للشات  🚦 .\n♪ مرحبا بك : @CRAZ_UP  🚦 .**")
    dev = await get_dev(bot_username)
...
async def CRAZ_UP(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
"مره واحد اسمو كريزي  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
"لا جوز كيان على الدوق فيما يحب أو بكره",
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝐒𝐎𝐔𝐑𝐂𝐄 𝐂𝐑𝐀𝐙𝐘](t.me/CR_OL_Q) 🚦 .
么  [𝒅𝒆𝒗 ᏨᎡᎯ ᏃᎽ ](t.me/CRAZ_UP) 🚦 .
╰──── • ◈ • ────╯
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        [
                            InlineKeyboardButton("SeMo ElKbEr 🥷", user_id=f"CRAZ_UP")
                        ],
                        [
...
@Client.on_message(filters.command(["صاحب تيم كريزي","الكريزي","المبرمج"], ""))
...
@Client.on_message(filters.command(["الجوز كيان","المطور حوده ","المطور جوز كيان"], ""))
...
async def booot(client: Client, message: Message):
    ...
    if not await is_served_chat(client, chat_id):
       try:
        ...
        await client.send_message(chat_id, f"**تم رفع البوت بنجاح✓ 🚦❤️‍🔥**")
        return 
       except:
          pass
    message.continue_propagation()
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["CRAZ_UP"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["CRAZ_UP"]:
   ...
   if message.from_user.username == "CRAZ_UP":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور كريزي  🚦 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @CR_OL_Q  🚦 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @CR_OL_Q  🚦 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @CR_OL_Q  🚦 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["CRAZ_UP"]:
         await message.reply_text("**♪ رتبتك هي : المطور كريزي+  🚦 .**")
         return
     if userr.username in ["CRAZ_UP"]:
         await message.reply_text("**♪ رتبتك هي : المطور جوز كيان 🚦 .**")
         return
     if userr.id == dev:
